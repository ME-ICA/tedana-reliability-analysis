...
subjs = glob(op.join('out', 'work', 'fmriprep_wf', 'single_subject_*'))

for s in subjs:
    runs = glob(op.join(s, 'func_preproc_task*'))
    for r in runs:
        t2smap_infiles = glob(op.join(r, 'join_echos', 'result_join_echos.pklz'))

        with gzip.open(t2smap_infiles[0]) as f:
            echo_list = pickle.load(f).outputs.bold_files

        t2smap_outdir = glob(op.join(r, 'bold_t2smap_wf', 't2smap_node',
        ...
        tfm = glob(o[.join(r, 'bold_mni_trans_wf', 'mask_merge_tfms',
